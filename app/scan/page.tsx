...
const SCANS = [
  ...
  { code: 'mri_whole_body', icon: '�ae1', name: 'Whole Body MRI', href: '/scan/whole-body-mri', from: 950, category: 'Magnetic Resonance', tagline: 'Complete screening — head to toe, no radiation', saving: 73 },
  { code: 'ct_angio', icon: '�ae0', name: 'CT Angiography', href: '/scan/ct-angio', from: 280, category: 'CT Imaging', tagline: 'Non-invasive coronary, pulmonary & carotid artery imaging', saving: 69 },
];
...
export default function ScanIndexPage() {
  return (
    <>
      <Navbar />
      <main>
        <section style={{ background: 'linear-gradient(135deg, var(--primary-3) 0%, var(--primary) 100%)', padding: '72px 24px 56px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}>
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(32px, 5vw, 56px)', color: '#fff', marginBottom: 16 }}>
              Advanced Medical Imaging
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 18, maxWidth: 560, margin: '0 auto' }}>
              Technologies not available on the NHS — or with 12–18 month waiting lists. Book within days.
            </p>
          </div>
        </section>
        ...
        <section style={{ padding: '64px 24px', background: 'var(--bg)' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20 }}>
              {SCANS.map(scan => (
                <Link key={scan.code} href={scan.href} style={{
                  ...
                  <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 22, color: 'var(--primary)', marginBottom: 8 }}>
                    {scan.name}
                  </h2>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--primary)' }}>£{scan.from.toLocaleString()}</span>
                    </div>
                    <span style={{ background: 'var(--accent-light)', color: 'var(--accent-2)', borderRadius: 100, padding: '3px 10px', fontSize: 12, fontWeight: 700 }}>
                      Save {scan.saving}%
                    </span>
                  </div>
                </Link>
