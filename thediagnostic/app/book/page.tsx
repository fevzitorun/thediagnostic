...
// Generate next 14 days of mock slots
...
function StepIndicator({ step }: { step: number }) {
  return (
    <div style={{ background: '#fff', borderBottom: '1px solid var(--line)', padding: '16px 24px', position: 'sticky', top: 64, zIndex: 10 }}>
          ...
          <div key={s.n} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS.length - 1 ? 1 : 0 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                ...
                background: step > s.n ? 'var(--accent)' : step === s.n ? 'var(--primary)' : 'var(--line)',
            ...
            {i < STEPS.length - 1 && (
              <div style={{
                ...
                background: step > s.n ? 'var(--accent)' : 'var(--line)',
...
function ScanCard({ scan, selected, onClick }: { scan: typeof SCAN_TYPES[0]; selected: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: selected ? 'var(--primary-05)' : '#fff',
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--primary)', marginBottom: 2 }}>{scan.name}</div>
          {scan.popular && (
            <span style={{ background: 'var(--accent)', color: '#fff', borderRadius: 100, padding: '2px 8px', fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
              POPULAR
            </span>
          )}
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>{scan.tagline}</div>
        <div style={{ fontSize: 14, fontWeight: 700, color: selected ? 'var(--primary)' : 'var(--text-muted)' }}>
...
function YesNoButton({ label, value, selected, onClick }: { label: string; value: boolean; selected: boolean | null; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: isSelected ? (value ? 'var(--accent)' : '#C0392B') : '#fff',
      color: isSelected ? '#fff' : 'var(--text-muted)',
      border: `1.5px solid ${isSelected ? (value ? 'var(--accent)' : '#C0392B') : 'var(--line)'}`,
...
export default function BookPage() {
  ...
  const NavButtons = ({ canNext, nextLabel = 'Continue →' }: { canNext: boolean; nextLabel?: string }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--line)' }}>
      {step > 1 ? (
        <button onClick={back} style={{ background: '#fff', border: '1.5px solid var(--line)', color: 'var(--text-muted)', borderRadius: 9, padding: '11px 22px', fontSize: 15, cursor: 'pointer' }}>
          ← Back
        </button>
      ) : <div />}
      ...
      <main style={{ background: 'var(--bg)', minHeight: '100vh', padding: '40px 24px 80px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          {step === 1 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 32, color: 'var(--primary-3)', marginBottom: 8 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
                {SCAN_TYPES.map(scan => (
                  <ScanCard
                    key={scan.code}
                    scan={scan}
                    selected={booking.scanType === scan.code}
                    onClick={() => update({ scanType: scan.code, scanName: scan.name })}
                  />
                ))}
              </div>

              <div style={{ marginTop: 20, padding: '14px 18px', background: 'var(--bg-2)', borderRadius: 10, fontSize: 13, color: 'var(--text-muted)' }}>
                💬 <strong>Not sure which scan you need?</strong>{' '}
                <a href="https://wa.me/447700000000" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', fontWeight: 600 }}>
                  Ask our clinical team on WhatsApp
                </a>
              </div>
          ...
          {step === 2 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 32, color: 'var(--primary-3)', marginBottom: 8 }}>
                  Where would you like to travel?
                </h2>
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  All destinations offer the same world-class imaging standards.
                </p>
              </div>
                  ...
                  return (
                    <button key={`${dest.country}-${dest.city}`}
                      ...
                      style={{
                        ...
                        background: selected ? 'var(--primary-05)' : dest.comingSoon ? 'var(--bg-2)' : '#fff',
                        border: `2px solid ${selected ? 'var(--primary)' : 'var(--line)'}`,
                        borderRadius: 12, cursor: dest.comingSoon ? 'default' : 'pointer',
                        opacity: dest.comingSoon ? 0.6 : 1,
                        transition: 'all 0.15s',
                      }}>
                      <div style={{ fontSize: 32, marginBottom: 10 }}>{dest.flag}</div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--primary)', marginBottom: 4 }}>{dest.label}</div>
                      {dest.comingSoon ? (
                        <div style={{ fontSize: 12, color: 'var(--warm)', fontWeight: 600 }}>Coming Soon</div>
                      ) : (
                      ...
                      {dest.popular && !dest.comingSoon && (
                        <div style={{ marginTop: 8, display: 'inline-block', background: 'var(--accent-light)', color: 'var(--accent-2)', borderRadius: 100, padding: '2px 8px', fontSize: 10, fontWeight: 700 }}>
                          MOST POPULAR
                        </div>
                      )}
                    </button>
          ...
          {step === 3 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  {booking.scanName} in {booking.city} — {CLINICS.filter(c => c.prices[booking.scanType] !== undefined).length} clinics available
                </p>
              </div>
                ...
                {CLINICS.filter(c => c.prices[booking.scanType] !== undefined).map(clinic => {
                  ...
                  return (
                    <button key={clinic.id}
                      ...
                      style={{
                        textAlign: 'left', background: selected ? 'var(--primary-05)' : '#fff',
                        border: `2px solid ${selected ? 'var(--primary)' : 'var(--line)'}`,
                      ...
                      <div style={{ padding: 22, display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'center' }}>
                        <div>
                          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                            {clinic.jci && <span style={{ background: 'var(--primary-05)', color: 'var(--primary)', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>JCI</span>}
                            {clinic.iso && <span style={{ background: 'var(--bg-2)', color: 'var(--text-muted)', borderRadius: 4, padding: '2px 8px', fontSize: 11 }}>ISO</span>}
                            <span style={{ background: 'var(--accent-light)', color: 'var(--accent-2)', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 600 }}>{clinic.highlight}</span>
                          </div>
                          <div style={{ fontWeight: 700, fontSize: 17, color: 'var(--primary)', marginBottom: 2 }}>{clinic.name}</div>
                          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8 }}>📍 {clinic.city}</div>
                          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                            {clinic.lead} · ⭐ {clinic.rating} · {clinic.turnaround}
                          </div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 2 }}>Price</div>
                          <div style={{ fontSize: 28, fontWeight: 700, color: selected ? 'var(--primary)' : 'var(--text)' }}>
                            £{price.toLocaleString()}
                          </div>
                          <div style={{ marginTop: 8, width: 24, height: 24, borderRadius: '50%', border: `2px solid ${selected ? 'var(--primary)' : 'var(--line)'}`, background: selected ? 'var(--primary)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: 'auto' }}>
                            {selected && <span style={{ color: '#fff', fontSize: 12 }}>✓</span>}
                          </div>
                        </div>
                      </div>
          ...
          {step === 4 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  Available slots at {booking.clinicName} — next 14 days
                </p>
              </div>
                  ...
                  {slots.map(slot => {
                    ...
                    return (
                      <button key={slot.date}
                        onClick={() => update({ selectedDate: slot.date, selectedTime: '' })}
                        style={{
                          ...
                          fontSize: 13, fontWeight: selected ? 700 : 400, textAlign: 'center',
                          minWidth: 80,
                        }}>
                        {slot.dateLabel}
                      </button>
              ...
              {booking.selectedDate && booking.selectedTime && (
                <div style={{ marginTop: 24, background: 'var(--accent-light)', border: '1.5px solid var(--accent)', borderRadius: 12, padding: '16px 20px' }}>
                  <div style={{ fontWeight: 700, color: 'var(--accent-2)', fontSize: 15, marginBottom: 4 }}>
                  ...
                  <div style={{ color: 'var(--text)', fontSize: 14 }}>
                    ...
                    <span style={{ color: 'var(--accent-2)', fontWeight: 700 }}>Price: £{clinicPrice.toLocaleString()}</span>
                  </div>
                </div>
              )}
          ...
          {step === 5 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  Required for the clinic to prepare your examination.
                </p>
              </div>

              {/* Personal info */}
              <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 28, marginBottom: 20 }}>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 20 }}>Personal Information</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  {[
                    ...
                    <div key={field.field}>
                      ...
                      <input
                        type={field.type}
                        value={booking[field.field as keyof BookingState] as string ?? ''}
                        onChange={e => update({ [field.field]: e.target.value } as Partial<BookingState>)}
              ...
              {/* MRI / Scan-specific safety */}
              {['mri_3t', 'mri_whole_body', 'pet_mri'].includes(booking.scanType) && (
                <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 28, marginBottom: 20 }}>
                  <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 6 }}>MRI Safety Screening</h3>
                  ...
                  {[
                    ...
                    <div key={q.field} style={{ marginBottom: 20 }}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', marginBottom: 10 }}>{q.label}</div>
                      ...
                      {booking[q.field] === true && q.warn && (
                        <div style={{ marginTop: 10, background: '#FEF9F0', border: '1.5px solid var(--warm)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#7D5A00' }}>
              ...
              <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 28 }}>
                <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 6 }}>Medical History (Optional)</h3>
                <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>
                  Any allergies, current medications, previous scan reports, or clinical indication for this scan.
                </p>
                <textarea
                  value={booking.medicalNotes}
                  onChange={e => update({ medicalNotes: e.target.value })}
                  placeholder="e.g. Allergic to iodine contrast. Previous CT chest 2024. Referred by Dr. Smith for oncology staging."
                  rows={4}
                  style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1.5px solid var(--line)', fontSize: 14, resize: 'vertical', background: '#fff', color: 'var(--text)', boxSizing: 'border-box' }}
                />
              </div>
          ...
          {step === 6 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  Optional. We can arrange everything — you just show up for your scan.
                </p>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 24 }}>
                {CONCIERGE_ITEMS.map(item => {
                  ...
                  return (
                    <button key={item.key}
                      onClick={() => update({ [item.key]: !checked } as Partial<BookingState>)}
                      style={{
                        textAlign: 'left', padding: '18px 22px',
                        background: checked ? 'var(--bg-2)' : '#fff',
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--primary)', marginBottom: 2 }}>{item.label}</div>
              ...
              {/* Departure city (shown if flight or transfer selected) */}
              ...
              {booking.translatorNeeded && (
                <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 12, padding: 22, marginBottom: 14 }}>
                  <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', display: 'block', marginBottom: 8 }}>
                    Preferred Language for Translator
                  </label>
                  <select
                    value={booking.translatorLanguage}
                    onChange={e => update({ translatorLanguage: e.target.value })}
                    style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1.5px solid var(--line)', fontSize: 14, background: '#fff', color: 'var(--text)' }}>
                    <option value="">Select language...</option>
                    {['Arabic', 'German', 'French', 'Dutch', 'Swedish', 'Russian', 'Italian', 'Spanish'].map(l => (
          ...
          {step === 7 && (
            <div>
              <div style={{ marginBottom: 32 }}>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: 15 }}>
                  Check your booking details and complete payment to confirm your slot.
                </p>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, alignItems: 'start' }}>
                ...
                <div>
                  ...
                  <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 24, marginBottom: 20 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 18 }}>Booking Summary</h3>
                    {[
                      ...
                      <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--line)' }}>
                        ...
                        <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{row.value}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 24 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 16 }}>Payment Currency</h3>
                    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                      {CURRENCIES.map(c => (
                        <button key={c.code}
                          ...
                          style={{
                            ...
                            background: booking.currency === c.code ? 'var(--primary)' : '#fff',
                            color: booking.currency === c.code ? '#fff' : 'var(--text)',
                            border: `1.5px solid ${booking.currency === c.code ? 'var(--primary)' : 'var(--line)'}`,
                    ...
                    {booking.currency === 'TRY' && (
                      <div style={{ marginTop: 14, fontSize: 13, color: 'var(--text-muted)', background: 'var(--bg-2)', borderRadius: 8, padding: '10px 14px' }}>
                        ₺ Turkish Lira payments processed via İyzico (3D Secure).
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{ position: 'sticky', top: 120 }}>
                  <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, padding: 24, marginBottom: 16 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 700, color: 'var(--primary)', marginBottom: 18 }}>Price Breakdown</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>{booking.scanName}</span>
                        <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>£{clinicPrice.toLocaleString()}</span>
                      </div>
                      {booking.flightNeeded && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>Concierge (flight)</span>
                          <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>TBD</span>
                        </div>
                      )}
                      {booking.hotelNeeded && (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>Hotel (1 night)</span>
                          <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>TBD</span>
                        </div>
                      )}
                      <div style={{ borderTop: '1px solid var(--line)', paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                        <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Total Due Now</span>
                        <span style={{ fontSize: 26, fontWeight: 700, color: 'var(--primary)' }}>£{clinicPrice.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center', lineHeight: 1.6 }}>
                    ...
                    <br />
                    Your slot will be confirmed by email within 15 minutes.
                  </div>
                  ...
                  {step > 1 && (
                    <button onClick={back} style={{
                      ...
                      borderRadius: 9, padding: '11px 0', fontSize: 14, cursor: 'pointer',
                    }}>
                      ← Edit Booking
                    </button>
                  )}
